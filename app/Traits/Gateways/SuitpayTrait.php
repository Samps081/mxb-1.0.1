...
trait SuitpayTrait
{
    ...
    public static function requestQrcode($request)
    {
        ...
        $requestNumber = uniqid(mt_rand(), true);
        ...
        if($response->successful()) {
            ...
            self::generateTransaction($responseData['idTransaction'], \Helper::amountPrepare($request->amount), $request->accept_bonus, $requestNumber); /// gerando historico
    ...
    public static function consultStatusTransaction($request)
    {
        try {
            ...
            return response()->json(['error' => 'Ocorreu um erro durante a consulta de status da transação.'], 500);
        }
    }
     ...
     * @return bool
     */
    public static function finalizePayment($idTransaction, $requestNumber) : bool
    {
        $transaction = Transaction::where('payment_id', $idTransaction)->where('status', 0)->first();
        $setting = \Helper::getSetting();
    
        if($transaction->reference  == $requestNumber){
     ...
     * @return void
     */
    private static function generateTransaction($idTransaction, $amount, $accept_bonus, $referenceToken)
    {
        $setting = \Helper::getSetting();
